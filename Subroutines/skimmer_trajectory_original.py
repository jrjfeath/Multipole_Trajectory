...
def skimmer_trajectory(d):
    ...
    urnd = np.random.uniform(0, 1) #uniform random distribution
    ...
    if dist == 'g':
        ...
        ry = np.random.normal(cy, sigma) / 1000
        rz = np.random.normal(cz, sigma) / 1000

    #Calculate a uniform distribution with limits of skmr_r
    if dist == 'u':
        ry = cy + np.random.uniform(-skmr_r, skmr_r) / 1000
        rz = cz + np.random.uniform(-skmr_r, skmr_r) / 1000
